<script>
	...
	function next() {
		activeIndex = (activeIndex + 1) % teams.length;
		isHovered = false;
	}

	function prev() {
		activeIndex = (activeIndex - 1 + teams.length) % teams.length;
		isHovered = false;
	}
...
>
				...
				>
					...
					{#if i !== activeIndex}
						<button
							...
							aria-label="Visualizza {team.name}"
						></button>
					{/if}
				</div>
			{/each}
	...
	<!-- Dot Navigation: barra barra arrotondata con ombra e opacità al 40% -->
...
<style>
	...
	.dot-button.active {
		/* Trasformiamo il dot attivo in una barra allungata per visualizzare il progresso temporale */
		width: 40px;
