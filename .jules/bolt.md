...
## 2024-07-28 - Layout thrashing on touchmove with requestAnimationFrame throttle
**Learning:** Just like with `mousemove`, calling `getBoundingClientRect()` synchronously inside a high-frequency `touchmove` event handler causes severe layout thrashing, as it forces the browser to calculate layouts constantly. This is especially problematic on mobile devices where resources are constrained and touch events can fire very rapidly.
**Action:** Throttle the high frequency execution in `touchmove` using `requestAnimationFrame` to coalesce DOM layout reads.
