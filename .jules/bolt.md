...
## 2024-07-29 - Throttling generic state updates on mousemove
**Learning:** High-frequency events like `mousemove` can trigger hundreds of times per second. Even if the event handler doesn't read from the DOM (causing layout thrashing), updating state (like coordinates for custom WebGL renders or component positions like a tooltip) on every single event is inefficient because the screen only refreshes at 60Hz (or 120Hz). Doing work more frequently than the display can show it wastes CPU/battery and can lead to micro-stutters in complex applications.
**Action:** Throttle ALL high-frequency state updates in `mousemove` (or similar events) using `requestAnimationFrame`. This ensures the state is updated exactly once per display frame, aligning the application's logic loop perfectly with the browser's render loop.
