...
## 2024-07-08 - Redundant Window dimension checking in components
**Learning:** Using `window.innerWidth` and binding `resize` event listeners redundantly across multiple components adds overhead and reads the DOM unnecessarily, particularly inside hover effects or reactive variables.
**Action:** Use a centralized `matchMedia`-based global state (e.g. `mediaQuery.svelte.js`) and inject it across the app rather than writing `window.innerWidth` checks and `resize` listeners in individual components.
